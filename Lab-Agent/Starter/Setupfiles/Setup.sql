...
GO
ALTER AUTHORIZATION ON DATABASE::HumanResources TO [UCOM\Student];
GO
...
GO
ALTER AUTHORIZATION ON DATABASE::InternetSales TO [UCOM\Student];
GO
...
GO
GO
ALTER AUTHORIZATION ON DATABASE::AWDataWarehouse TO [UCOM\Student];
GO
...
IF EXISTS (select * FROM msdb.dbo.systargetservers  WHERE server_name = 'UCOM-SQL1\SQL2')
	EXECUTE msdb.dbo.sp_delete_targetserver @server_name = 'UCOM-SQL1\SQL2';
...
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Back Up Database - InternetSales', @server_name = N'UCOM-SQL1'
GO
...
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Back Up Database - InternetSales', @step_name=N'1', 
		...
		@command=N'BACKUP DATABASE [InternetSales] TO  DISK = N''InternetSales.bak'' WITH FORMAT, INIT,  MEDIANAME = N''InternetSales Baclups'',  NAME = N''InternetSales-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO
', 
...
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Back Up Log - InternetSales', @server_name = N'UCOM-SQL1'
GO
...
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Back Up Log - InternetSales', @step_name=N'1', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'BACKUP LOG [InternetSales] TO  DISK = N''InternetSales.bak'' WITH NOFORMAT, NOINIT,  NAME = N''InternetSales-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO
', 
		@database_name=N'master', 
		@flags=0
...
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Back Up Database - HumanResources', @server_name = N'UCOM-SQL1'
GO
USE [msdb]
GO
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Back Up Database - HumanResources', @step_name=N'1', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'BACKUP DATABASE [HumanResources] TO  DISK = N''HumanResources.bak'' WITH NOFORMAT, NOINIT,  NAME = N''HumanResources-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
GO
', 
		@database_name=N'master', 
		@flags=0
...
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Back Up Database - AWDataWarehouse', @server_name = N'UCOM-SQL1'
GO
