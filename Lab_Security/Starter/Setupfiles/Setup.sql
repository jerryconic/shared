...
IF EXISTS (SELECT * FROM sys.server_principals WHERE name = 'UCOM\WebApplicationSvc')
	DROP LOGIN [UCOM\WebApplicationSvc]
GO

IF EXISTS (SELECT * FROM sys.server_principals WHERE name = 'UCOM\IT_Support')
	DROP LOGIN [UCOM\IT_Support]
GO
...
GO
ALTER AUTHORIZATION ON DATABASE::AdventureWorks TO [UCOM\Student];
GO
